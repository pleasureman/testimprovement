...
后Devops时代指的是Devops已经和Docker容器技术充分的融合在一起应用的时代。在这个时代中，随着云计算的普及，很多云平台提供了应用引擎，如果你的应用符合引擎的规范，云平台就可以自动检测业务负载量。当业务出现峰值时，平台可以利用底层的Docker容器技术的快速部署、资源快速扩展伸缩等特性来应对，从而有效的支撑了业务的正常运行。<br>
