...
本文以linux内核为例，主要从以下几个方面来阐述对linux内核测试可进行的容器化改造。
1.	容器中并发执行内核功能测试。在同一台主机上可以同时运行数千个docker容器，在容器中并发运行内核功能测试套，可以大大减少测试执行时间，达到测试加速效果。不过这种方式仍有一定的局限性：
A．	在容器内执行内核测试时可能会有较多的失败用例，因为有某些测试需要提前进行适配工作，比如在测试内核namespace嵌套时，在容器内可嵌套的层数要比主机上少一层 ，诸如此类的测试套需在执行前提前进行适配。
