...
### 2.1.Kernel开源社区
Linux kernel是迄今为止最为成功的开源社区，有很多一流的公司和程序员参与其中，平均每月的代码补丁合入的补丁数量在一千以上，社区的代码量在千万行的数量级。面对如此庞大的代码量，质量保障工作是一个严峻的挑战。而目前社区没有精准测试方案，修改一行开发代码时不得不运行全量的测试用例才能充分保证质量。
