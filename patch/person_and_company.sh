...
REPORT=$TMP/qemu.html
TOPDIR=`dirname $0`
cd $TOPDIR;
TOPDIR=`pwd`/community
echo "TOPDIR= $TOPDIR"
#otherparameter="--since 2015-1-1 --until 2015-12-31 --pretty=format:\"%ae\""
#otherparameter="--since 2015-1-1 --until 2015-12-31 --pretty=format:%ae"
otherparameter="--pretty=format:%ae"
mkdir -p $TOPDIR
#TOPDIR=/root/patch/community/
#communitylist="ltp libnetwork libcontainer"
#communitylist="kernel docker runc ltp swarm distribution libnetwork machine kpatch logrus coder mcelog syslog-ng docker-bench-security notary f2fs-tools specs nfs-ganesha openembedded-core meta-openembedded uclibc-ng busybox"
communitylist="qemu"
...
do
	...
	if [ -e ${TOPDIR}/$i ];then
		...
		"specs" ) repo="https://github.com/opencontainers/specs.git";;
		"nfs-ganesha" ) repo="https://github.com/nfs-ganesha/nfs-ganesha.git";;
		"openembedded-core" ) repo="http://git.openembedded.org/openembedded-core";;
		"meta-openembedded" ) repo="http://git.openembedded.org/meta-openembedded";;
		"uclibc-ng" ) repo="http://git.uclibc-ng.org/uclibc-ng.git";;
		"busybox" ) repo="http://git.busybox.net/busybox";;
		"qemu" ) repo="git://git.qemu.org/qemu.git";;
		#"swarm" ) repo="https://github.com/docker/swarm.git";;
	...
	git pull 
	#git log ${otherparameter} | grep ^Author: | grep -e "@huawei.com" -e "@hisilicon.com" -e "kaixu.xia@linaro.org" -e "hanjun.guo@linaro.org" | awk '{print $NF}' | sed 's/^<//g' | sed 's/>$//g' | grep -v ^$ | grep -v "\?" > ${TMP}/${i}_authors || echo error1
	git log ${otherparameter} | grep -e "@huawei.com" -e "@hisilicon.com" -e "kaixu.xia@linaro.org" -e "hanjun.guo@linaro.org" | grep -v ^$ | grep -v "\?" > ${TMP}/${i}_authors || echo error1
	git log ${otherparameter} | grep -v ^$ | grep -v "\?" | awk -F "@" '{print $2}' | sed '/^$/d' > ${TMP}/${i}_company || echo error1
	echo "sort $i data"
